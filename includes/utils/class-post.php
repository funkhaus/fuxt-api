    ...
    public static function get_postdata($post, $additional_fields = array(), $params = array())
    {
        ...
        if (! empty($additional_fields) && is_array($additional_fields)) {
            ...
            if (in_array('terms', $additional_fields)) {
                $include_acf_in_terms = in_array('acf', $additional_fields);
                $taxonomies           = get_object_taxonomies($post->post_type, 'names');
                foreach ($taxonomies as $taxonomy) {
                    $terms                      = get_the_terms($post->ID, $taxonomy);
                    $data['terms'][$taxonomy] = $terms ? array_map(
                        function ($term) use ($include_acf_in_terms) {
                            return Utils::get_termdata($term, $include_acf_in_terms);
                        },
                        $terms
                    ) : null;
                }
        ...
        foreach ((array) $rewrite as $match => $query) {
            ...
            if (preg_match("#^$match#", $request_match, $matches)) {
                ...
                if ($wp_rewrite->use_verbose_page_rules && preg_match('/pagename=\$matches\[([0-9]+)\]/', $query, $varmatch)) {
                    ...
                    if (
                        $page->post_status !== 'draft' && ! $post_status_obj->public && ! $post_status_obj->protected
